...
const els = {
  openBtn:$('openBtn'), mergeBtn:$('mergeBtn'), insertBtn:$('insertBtn'), wordBtn:$('wordBtn'), exportBtn:$('exportBtn'),
  fileInput:$('fileInput'), mergeInput:$('mergeInput'), insertInput:$('insertInput'),
...
function restore(snap){state.pages=deepClone(snap.pages);state.pageNumber=deepClone(snap.pageNumber||{enabled:false,position:'center',format:'number',fromPage:1,start:1,fontSize:11,margin:20,color:'#333333'});state.currentIndex=clamp(snap.currentIndex,0,Math.max(0,state.pages.length-1));state.selectedAnnotationId=null;state.selectedAnnotationIds=new Set();state.pendingTextHit=null;rebuildThumbs();renderContinuousDocument({scrollToCurrent:true});updateUI();scheduleAutosave()}
...
async function openFile(file){
  ...
  try{
    const {source,entries}=await loadPdfSource(file);await recoveryClear();state.sources=[source];state.pages=entries;state.currentIndex=0;state.undo=[];state.redo=[];state.zoom=1.15;state.selectedAnnotationId=null;state.selectedAnnotationIds=new Set();state.pageNumber={enabled:false,position:'center',format:'number',fromPage:1,start:1,fontSize:11,margin:20,color:'#333333'};state.textCache.clear();state.fileName=file.name.replace(/\.pdf$/i,'')+'_edited.pdf';state.recoveryDisabled=false;state.recoverySourceSignature='';
...
function updateUI(){
  ...
  if(els.addPageBtn)els.addPageBtn.disabled=!has;if(els.saveProjectBtn)els.saveProjectBtn.disabled=!has;
...
function renderAnnotationsForPage(index,overlay){
  ...
  for(const a of entry.annotations){
    ...
    const label=document.createElement('span');label.className='annotation-label';label.textContent=(a.locked?'🔒 ':'')+(a.type==='replace'?'修改文字':a.type==='watermark'?'浮水印':a.type==='text'?'新增文字':a.type==='redact'?'永久塗銷':'色塊');d.appendChild(label);const h=document.createElement('span');h.className='resize-handle';d.appendChild(h);d.addEventListener('pointerdown',annotationPointerDown);d.addEventListener('dblclick',annotationDoubleClick);h.addEventListener('pointerdown',resizePointerDown);overlay.appendChild(d)
  }
...
async function saveProjectFile(){if(!state.pages.length||state.operation)return;const total=totalSourceBytes();if(total>450*1024*1024&&!confirm(`此專案來源 PDF 約 ${(total/1024/1024).toFixed(0)} MB，儲存 .govpdf 可能需要較多記憶體與時間。\n\n仍要繼續嗎？`))return;const op=startOperation('project-save',false);setBusy(true,'正在儲存 GovPDF 專案','打包來源 PDF 與可編輯物件…',{progress:true});try{const JSZip=await ensureJSZip(),zip=new JSZip(),sources=[];for(let i=0;i<state.sources.length;i++){const s=state.sources[i],path=`sources/source_${String(i+1).padStart(3,'0')}.pdf`;sources.push({id:s.id,name:s.name,path});zip.file(path,new Uint8Array(s.bytes),{compression:'STORE'});setBusyProgress(i+1,state.sources.length,`加入來源 ${i+1} / ${state.sources.length}`);await nextFrame()}const manifest={format:'GovPDFProject',formatVersion:1,appVersion:APP_VERSION,savedAt:new Date().toISOString(),fileName:state.fileName,currentIndex:state.currentIndex,zoom:state.zoom,performanceMode:state.performanceMode,pageNumber:deepClone(state.pageNumber),pages:deepClone(state.pages),sources};zip.file('project.json',JSON.stringify(manifest,null,2));const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:3}},m=>setBusyProgress(m.percent,100,`壓縮專案 ${Math.round(m.percent)}%`));const base=(state.fileName||'GovPDF_Project').replace(/_edited\.pdf$/i,'').replace(/\.pdf$/i,'');downloadBlob(blob,`${base}.govpdf`);showToast('GovPDF 專案已儲存')}catch(e){console.error(e);alert(`儲存專案失敗：\n${e?.message||e}`)}finally{setBusy(false)}}
...
async function renderNonDestructiveOverlayCanvas(entry,index,scale=2.2){const srcPage=await getPdfJsPage(entry),base=srcPage.getViewport({scale:1,rotation:0}),c=document.createElement('canvas');c.width=Math.max(1,Math.ceil(base.width*scale));c.height=Math.max(1,Math.ceil(base.height*scale));const ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);const annotations=(entry.annotations||[]).filter(a=>!annotationRequiresSecureFlatten(a));if(annotations.length)drawAnnotationsToCanvas(ctx,c.width,c.height,annotations,scale);if(pageNumberText(index)&&state.pageNumber.format==='zhTotal')drawPageNumberToCanvas(ctx,c.width,c.height,index,scale);return{canvas:c,base}}
async function renderEntryCanvas(entry,scale=1.8,withAnnotations=true){const srcPage=await getPdfJsPage(entry),base=srcPage.getViewport({scale:1,rotation:entry.rotation}),vp=srcPage.getViewport({scale,rotation:entry.rotation}),c=document.createElement('canvas');c.width=Math.ceil(vp.width);c.height=Math.ceil(vp.height);const ctx=c.getContext('2d',{alpha:false});await srcPage.render({canvasContext:ctx,viewport:vp,background:'white'}).promise;if(withAnnotations)drawAnnotationsToCanvas(ctx,c.width,c.height,entry.annotations,scale);return{canvas:c,base,vp}}
function drawAnnotationsToCanvas(ctx,w,h,annotations,scale){
  ctx.save();for(const a of annotations){const x=a.x*w,y=a.y*h,aw=a.w*w,ah=a.h*h;if(a.type==='rect'||a.type==='redact'){ctx.save();ctx.fillStyle=a.type==='redact'?'#000000':(a.color||'#fff');ctx.fillRect(x,y,aw,ah);ctx.restore();continue}if(a.type==='replace'){ctx.save();ctx.fillStyle=a.bg||'#fff';ctx.fillRect(x,y,aw,ah);ctx.restore()}drawTextAnnotation(ctx,a,x,y,aw,ah,scale)}ctx.restore();
}
...
/* V3.6.3.1 Security & Reliability Final Hotfix ---------------------------------
...
const V363_ALLOWED_ANNOTATION_TYPES = new Set(['rect','text','replace','watermark','redact']);
